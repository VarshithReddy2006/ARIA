...
class RepositorySnapshot(BaseModel):
    ...
    indexed_timestamp: float = Field(..., description="Epoch timestamp of analysis build.")
...
class RepositoryTwin(BaseModel):
    ...
    snapshot: RepositorySnapshot = Field(..., description="Specific repository state pinning.")
    metadata: Dict[str, Any] = Field(..., description="Tech stack, LOC, general info.")
    files: List[str] = Field(..., description="List of file paths.")
    symbols_summary: Dict[str, Any] = Field(..., description="Total symbols count, public/private ratio.")
    dependencies_summary: Dict[str, Any] = Field(..., description="External packages and import relationships count.")
    architecture_summary: Dict[str, Any] = Field(..., description="Cycles, strongly connected components count, entry points.")
    health_summary: Dict[str, Any] = Field(..., description="Overall health score and grade breakdown.")
    compliance_summary: Dict[str, Any] = Field(..., description="Compliance status based on codebase rules.")
...
class RepositoryTwinSummary(BaseModel):
    ...
    tech_stack: List[str] = Field(..., description="Primary languages/technologies detected.")
    ...
    compliance_status: str = Field(..., description="Compliance status (compliant/warning/non-compliant).")
