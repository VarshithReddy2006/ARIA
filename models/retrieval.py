...
class ContextReference(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Entity type: file | symbol | component | health | compliance | document.",
    )
    source: str = Field(
        ...,
        description="Executor source: subgraph | symbol_expansion | dependency_expansion | embedding.",
    )
    properties: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata properties of the entity."
    )
    snippet: Optional[str] = Field(
        None,
        description="Optional raw text snippet (e.g. source snippet or documentation content).",
    )
...
class RetrievalPlanStep(BaseModel):
    ...
    executor: str = Field(
        ...,
        description="Executor identifier: subgraph | symbol | dependency | embedding.",
    )
    targets: List[str] = Field(
        default_factory=list, description="Target entity IDs for this step."
    )
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Configuration parameters for the executor."
    )
...
class RetrievalPlan(BaseModel):
    ...
    steps: List[RetrievalPlanStep] = Field(
...
class RetrievalExplanation(BaseModel):
    ...
    resolved_entities: List[str] = Field(
        ..., description="Matched code entity names from user question."
    )
    policy: str = Field(..., description="Selected retrieval policy.")
    confidence: float = Field(
        ..., description="Calculated retrieval confidence score (0.0-1.0)."
    )
    metrics: Dict[str, Any] = Field(
        default_factory=dict, description="Summary counts of retrieved elements."
    )
...
class RepositoryRetrievalContext(BaseModel):
    ...
    references: List[ContextReference] = Field(
        default_factory=list, description="Ranked stable context references."
    )
    subgraph: Optional[Dict[str, Any]] = Field(
        None, description="Composed semantic subgraph slice."
    )
    explanation: RetrievalExplanation = Field(
        ..., description="Explainability details."
    )
