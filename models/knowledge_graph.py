...
class KnowledgeGraphNode(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Entity type: repository | directory | file | symbol | component | health | compliance.",
    )
...
class KnowledgeGraphEdge(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Relationship type: CONTAINS | DECLARES | IMPORTS | CALLS | HAS_HEALTH | HAS_COMPLIANCE.",
    )
    properties: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata properties of the relationship."
    )
...
class KnowledgeGraph(BaseModel):
    ...
    nodes: List[KnowledgeGraphNode] = Field(
        default_factory=list, description="All semantic nodes."
    )
    edges: List[KnowledgeGraphEdge] = Field(
        default_factory=list, description="All semantic edges."
    )
...
class KnowledgeGraphSummary(BaseModel):
    ...
    node_types_breakdown: Dict[str, int] = Field(
        ..., description="Breakdown of nodes count by entity type."
    )
    edge_types_breakdown: Dict[str, int] = Field(
        ..., description="Breakdown of edges count by relationship type."
    )
