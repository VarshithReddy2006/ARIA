...
class RepositorySnapshot(BaseModel):
    ...
    snapshot_id: str = Field(
        ..., description="Unique snapshot identifier (e.g. repo_name_commit_sha)."
    )
    ...
    digital_twin_reference: str = Field(
        ..., description="Reference metadata identifier for the digital twin."
    )
    knowledge_graph_reference: str = Field(
        ..., description="Reference metadata identifier for the knowledge graph."
    )
    health_reference: str = Field(
        ..., description="Reference identifier for health details."
    )
    metrics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Aggregated metrics (complexity, symbol count, files, etc.).",
    )
...
class RepositoryEvent(BaseModel):
    ...
    event_type: str = Field(
        ...,
        description="FileAdded | FileRemoved | FileModified | SymbolAdded | SymbolRemoved | DependencyAdded | DependencyRemoved | ArchitectureChanged | HealthChanged | ComplianceChanged | ComplexityChanged",
    )
    affected_entity: str = Field(
        ..., description="Target file path, symbol name, or dependency ID."
    )
    ...
    severity: str = Field(
        "info", description="Change severity: info | warning | critical."
    )
    confidence: float = Field(1.0, description="Confidence score (0.0-1.0).")
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Extra event properties."
    )
...
class RepositoryTimeline(BaseModel):
    ...
    snapshots: List[RepositorySnapshot] = Field(
        default_factory=list, description="Immutable snapshots chronologically ordered."
    )
    events: List[RepositoryEvent] = Field(
        default_factory=list, description="Chronological list of change events."
    )
...
class TrendMetric(BaseModel):
    ...
    metric_name: str = Field(
        ..., description="Name of the evaluated metric (e.g. complexity, health_score)."
    )
    direction: str = Field(
        ..., description="Linear direction: Increasing | Decreasing | Stable."
    )
...
class ComparisonResult(BaseModel):
    ...
    changes: List[RepositoryEvent] = Field(
