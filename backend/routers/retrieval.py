...
class RetrievalRequest(BaseModel):
    ...
    question: str = Field(..., description="The user query or question about the codebase.")
    policy: str = Field("default", description="Retrieval policy: default | architecture | implementation | impact | security | performance")


@router.post("/repositories/{username}/{repository}/retrieve", response_model=RepositoryRetrievalContext)
async def retrieve_context(username: str, repository: str, request: RetrievalRequest):
    ...
    try:
        ...
        if repo_name not in structural_retrieval_engine.navigator.get_builder().twin_builder.store:
        ...
        logger.error("Structural retrieval failed for %s with query '%s': %s", repo_name, request.question, exc, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Retrieval execution failed: {str(exc)}")
