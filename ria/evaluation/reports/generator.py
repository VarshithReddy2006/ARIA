...
class ReportGenerator:
    ...
    def generate_benchmark_report(cls, results: list[BenchmarkResult]) -> str:
        ...
        for r in results:
            ...
            lines.append("| Stage | Elapsed (s) | Peak Memory (KB) | Throughput | Failures |")
            lines.append("|---|---|---|---|---|")
            for sm in r.stage_metrics:
                peak_kb = sm.peak_memory_bytes / 1024
                lines.append(f"| {sm.stage_name} | {sm.elapsed_seconds:.4f} s | {peak_kb:.1f} KB | {sm.throughput_per_sec:.1f}/s | {sm.failures} |")
            lines.append("")
