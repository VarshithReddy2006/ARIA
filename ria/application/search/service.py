...
class SearchApplicationService:
    ...
    def search_symbol(
        ...
        st = next(
            (
                ...
                if s.identity.repo_id.value == repo_id_str
            ),
            None,
        )
    ...
    def search_file(
        self, repo_id_str: str, query_text: str, max_results: int = 50
    ) -> SearchResponse:
        st = next(
            (
                s
        ...
        query = SearchQuery(
            query_text=query_text,
            query_type=SearchQueryType.FILE,
            options=SearchOptions(max_results=max_results),
        )
        return self._engine.search(
            query, self._fact_store, st.identity, st.current_commit
    ...
    def search_module(
        ...
        query = SearchQuery(
            query_text=query_text,
            query_type=SearchQueryType.MODULE,
            options=SearchOptions(max_results=max_results),
        )
        return self._engine.search(
            query, self._fact_store, st.identity, st.current_commit
    ...
    def autocomplete(
        self, repo_id_str: str, prefix: str, max_suggestions: int = 10
    ) -> SearchResponse:
        st = next(
            (
                s
        ...
        query = SearchQuery(
            query_text=prefix,
            query_type=SearchQueryType.AUTOCOMPLETE,
            options=SearchOptions(max_results=max_suggestions),
        )
        return self._engine.search(
            query, self._fact_store, st.identity, st.current_commit
        )
