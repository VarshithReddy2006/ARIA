...
class CLIRunner:
    ...
    def run(self, args: Sequence[str]) -> int:
        ...
        if parsed.command == "init":
            reg_dto = self._sync.register_repository(RegisterRepositoryCommand(remote_url=parsed.remote_url, name=parsed.name))
            ConsoleFormatter.print_success(f"Registered repo {reg_dto.repo_id} ({reg_dto.status})")
            return 0

        if parsed.command in ("index", "update"):
            upd_dto = self._sync.synchronize_repository(SynchronizeRepositoryCommand(repo_id=parsed.repo_id))
            ConsoleFormatter.print_success(f"Synchronized repo {upd_dto.repo_id} ({upd_dto.current_commit_sha})")
        ...
        if parsed.command == "query":
            q_res = self._query.find_definition(parsed.repo_id, symbol_moniker=parsed.symbol)
            ConsoleFormatter.print_success(f"Query returned ID {q_res.query_id}")
            return 0 if q_res.is_success else 1

        if parsed.command == "context":
            ctx_dto = self._context.build_context(BuildContextCommandDTO(repo_id=parsed.repo_id, question=parsed.question, max_tokens=parsed.max_tokens))
            ConsoleFormatter.print_success(f"Context package {ctx_dto.package_id} built with {ctx_dto.total_tokens} tokens")
            return 0 if ctx_dto.is_success else 1

        if parsed.command == "ask":
            know_dto = self._knowledge.answer_question(AnswerQuestionCommandDTO(repo_id=parsed.repo_id, question=parsed.question))
        ...
        if parsed.command in ("doctor", "config", "status"):
            ConsoleFormatter.print_info(f"Command '{parsed.command}' executed successfully.")
            return 0
