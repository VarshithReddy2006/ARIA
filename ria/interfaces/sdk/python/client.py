...
class RIAClient:
    ...
    def search(self, repo_id: str, query_text: str) -> SDKResponse:
        ...
        return SDKResponse(is_success=s_resp.is_success, data={"total_matches": matches_cnt})

    def query(self, repo_id: str, query_type: str, symbol_moniker: Optional[str] = None) -> SDKResponse:
        q_res = self._query.find_definition(repo_id, symbol_moniker=symbol_moniker)
        return SDKResponse(is_success=q_res.is_success, data={"query_id": q_res.query_id})

    def context(self, repo_id: str, question: str) -> SDKResponse:
        ctx_dto = self._context.build_context(BuildContextCommandDTO(repo_id=repo_id, question=question))
        return SDKResponse(is_success=ctx_dto.is_success, data={"package_id": ctx_dto.package_id, "content": ctx_dto.content})

    def ask(self, repo_id: str, question: str) -> SDKResponse:
        know_dto = self._knowledge.answer_question(AnswerQuestionCommandDTO(repo_id=repo_id, question=question))
        return SDKResponse(is_success=know_dto.is_success, data={"answer": know_dto.answer_text, "grounded": know_dto.is_grounded})
    ...
    def update(self, repo_id: str) -> SDKResponse:
        ...
        return SDKResponse(is_success=upd_dto.is_success, data={"commit_sha": upd_dto.current_commit_sha})

    def status(self, repo_id: str) -> SDKResponse:
        return SDKResponse(is_success=True, data={"repo_id": repo_id, "status": "active"})
