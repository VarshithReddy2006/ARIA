...
from ria.agent.exceptions import ToolNotFoundException
...
class ToolRegistry(ToolRegistryPort):
    ...
    def invoke_tool(
        ...
        if handler is None:
            ...
            return ToolExecution(tool_name=tool_name, parameters=parameters, output=output, is_success=True)

        try:
            out = handler(parameters)
            return ToolExecution(tool_name=tool_name, parameters=parameters, output=out, is_success=True)
        except Exception as err:
            out = {"error": str(err), "is_success": False}
            return ToolExecution(tool_name=tool_name, parameters=parameters, output=out, is_success=False)
