...
class MockLLMProvider(LLMProviderPort):
    ...
    def generate_response(
        ...
        ans_text = (
            ...
            f"1. Verified implementation details from context snippets.\n"
            f"2. Reference and symbol citations extracted directly from context."
        )
        total_tokens = len(prompt.system_prompt + prompt.user_prompt + ans_text) // 4
        return ProviderResponse(raw_text=ans_text, model=config.model_name, total_tokens=total_tokens)
