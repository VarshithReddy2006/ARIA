...
os.environ["ALLOWED_HOSTS"] = (
    '["aria.onrender.com", "localhost", "127.0.0.1", "testserver"]'
)
...
from backend.security_middleware import SESSION_COOKIE_NAME, verify_session_token  # noqa: E402
...
def run_verification():
    ...
    print(
        f"[*] 2. Direct Unauthenticated GET /api/v1/repos/examples -> Status: {res_unauth.status_code}"
    )
    ...
    print(
        f"[*] 3. X-API-Key GET /api/v1/repos/examples -> Status: {res_apikey.status_code}"
    )
    ...
    print(
        f"[*] 4. Bearer GET /api/v1/repos/examples -> Status: {res_bearer.status_code}"
    )
    ...
    print(
        f"[*] 6. Browser Cookie-Auth GET /api/v1/repos/examples -> Status: {res_cookie_auth.status_code}"
    )
    assert res_cookie_auth.status_code == 200, (
        f"Expected 200, got {res_cookie_auth.status_code}"
    )
    ...
    with unittest.mock.patch(
        ...
        with unittest.mock.patch(
            "backend.routers.repositories.snapshot_store.load",
            return_value={"summary": "ok", "relationships": [], "reading_order": []},
        ):
            ...
            print(
                f"[*] 7. Same-Origin Cookie POST /api/v1/analyze -> Status: {res_mutation_same.status_code}"
            )
            assert res_mutation_same.status_code in (200, 308), (
                f"Expected 200, got {res_mutation_same.status_code}"
            )

    # 8. Verify CSRF: Cross-Origin Mutation Rejected (403)
    attacker_client = TestClient(
        app,
        base_url="https://aria.onrender.com",
    ...
    print(
        f"[*] 8. Cross-Origin Cookie POST /api/v1/analyze -> Status: {res_mutation_cross.status_code}"
    )
    assert res_mutation_cross.status_code == 403, (
        f"Expected 403, got {res_mutation_cross.status_code}"
    )
    ...
    print(
        f"    qdrant_api_key = {'*' * len(settings.qdrant_api_key) if settings.qdrant_api_key else None}"
    )
    ...
    print(
        "    QdrantStore initialized with remote cloud URL, API key, and timeout verified successfully."
    )
