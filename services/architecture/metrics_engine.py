...
def compute_metrics(
    ...
    decision_keywords = ["if ", "elif ", "else:", "for ", "while ", "except ", "catch ", "case ", "&&", "||", " and ", " or "]
    ...
    public_symbols_count = len(re.findall(r"\b(export\s+|def\s+[a-zA-Z0-9]+|class\s+[a-zA-Z0-9]+)", content))

    abstract_count = len(re.findall(r"\b(ABC|abstractclass|interface|abstract)\b", content))
    abstractness = round(abstract_count / max(classes_count, 1), 3) if classes_count > 0 else 0.0
    distance_main_sequence = round(abs(abstractness + instability - 1.0), 3)

    # Maintainability Index calculation
    halstead_volume = max(loc * 5.0, 1.0)
    mi_raw = 171.0 - 5.2 * math.log(halstead_volume) - 0.23 * cyclomatic_complexity - 16.2 * math.log(loc)
