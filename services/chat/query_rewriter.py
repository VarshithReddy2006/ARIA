...
class QueryRewriter:
    ...
    def rewrite(
        ...
        if (switch_res and switch_res.is_topic_switch) or (explicit_res and explicit_res.has_explicit_entity):
            target_file = (switch_res.target_file if switch_res and switch_res.target_file else (explicit_res.target_file if explicit_res else None))
            target_symbol = (switch_res.target_symbol if switch_res and switch_res.target_symbol else (explicit_res.target_symbol or explicit_res.entity_name if explicit_res else None))
            target_anchor = target_file or target_symbol

            if target_anchor:
                current_file = context.current_file

                if "compare to" in q_lower or "differs" in q_lower or "versus" in q_lower:
                    if current_file and current_file.lower() != target_anchor.lower():
                        return f"How does {current_file} compare to {target_anchor}?"

                if q_lower.startswith(("explain", "now explain", "switch to", "move to", "focus on", "describe")):
                    return f"Explain {target_anchor}"

                if "belong there instead" in q_lower or "belong in" in q_lower or "belong there" in q_lower:
                    if current_file and current_file.lower() != target_anchor.lower():
        ...
        target_file = (switch_res.target_file if switch_res and switch_res.target_file else context.current_file)
        target_symbol = (switch_res.target_symbol if switch_res and switch_res.target_symbol else context.current_symbol)
        anchor = target_file or target_symbol or context.current_repo or ""

        if not anchor:
            return q_clean

        last_canonical = context.canonical_resolved_queries[-1] if context.canonical_resolved_queries else ""

        # Handle single-word / ultrashort follow-ups ("Why?", "How?", "Where?")
        if q_lower in ("why?", "why", "how?", "how", "where?", "where"):
            if "startup strategy" in last_canonical.lower() or "startup" in last_canonical.lower():
                return f"Why is the startup strategy implemented in {anchor} beneficial?"
            elif "services" in last_canonical.lower() or "initialize" in last_canonical.lower():
                return f"Why are the services initialized inside {anchor} during startup?"
            elif last_canonical:
        ...
        if "there" in q_lower or "that file" in q_lower or "belong there" in q_lower:
            prev_file = context.navigation_graph.find_previous_file() or (context.recently_discussed_files[0] if context.recently_discussed_files else None)
        ...
        if "services" in q_lower and ("initialize" in q_lower or "it initialize" in q_lower):
        ...
        if "how does it work" in q_lower or "how does it differ" in q_lower or "why is it separated" in q_lower:
            if "separated" in q_lower:
                return f"Why is {anchor} separated into its own module?"
            if "differ" in q_lower:
                prev = context.navigation_graph.find_previous_file() or (context.recently_discussed_files[0] if context.recently_discussed_files else None)
        ...
        if followup_res and followup_res.is_followup and anchor and anchor.lower() not in q_lower:
