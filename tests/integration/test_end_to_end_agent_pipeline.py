...
from ria.application.agent import (
...
def test_full_agent_runtime_end_to_end_pipeline(tmp_path: Path) -> None:
    ...
    subprocess.run(
        ["git", "commit", "-m", "Agent test commit"], cwd=origin_dir, check=True
    )
    ...
    status_dto = reg_use_case.execute(
        RegisterRepositoryCommand(remote_url=str(origin_dir), name="agent_origin")
    )
    ...
    for tool_meta in mcp_server.list_tools():
        ...
        tool_registry.register_tool(
            t_name, lambda params, name=t_name: mcp_server.invoke_tool(name, params)
        )
    ...
    agent_app_service = AgentApplicationService(
        agent_runtime,
        goal_interpreter,
        container.repository_registry,
        container.clock,
    ...
    # Warmup
    _ = execute_goal_uc.execute(
        ExecuteGoalCommandDTO(
            repo_id=status_dto.repo_id,
            goal_description="Explain AgentCore architecture",
        )
    )

    # 5. Measure End-to-End Performance
    t0 = time.perf_counter()
    res_dto = execute_goal_uc.execute(
        ExecuteGoalCommandDTO(
            repo_id=status_dto.repo_id,
            goal_description="Explain AgentCore architecture",
        )
    ...
    assert elapsed_ms < 50.0, (
        f"Agent Runtime execution latency {elapsed_ms:.2f}ms exceeded target threshold"
    )
