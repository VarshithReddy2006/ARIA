...
from ria.domain.search.entities import AutocompleteResult, SymbolResult
...
def test_full_search_engine_end_to_end_pipeline(tmp_path: Path) -> None:
    ...
    py_file.write_text("class UserRepository:\n    def find_by_id(self, user_id: str) -> str:\n        return 'user'\n")

    subprocess.run(["git", "add", "."], cwd=origin_dir, check=True)
    subprocess.run(["git", "commit", "-m", "Search test commit"], cwd=origin_dir, check=True)
    ...
    status_dto = reg_use_case.execute(RegisterRepositoryCommand(remote_url=str(origin_dir), name="search_origin"))
    ...
    search_engine = SearchEngine(planner, index, ranking, filters, highlight, autocomplete, cache)
    ...
    # Warm-up call
    _ = search_sym_uc.execute(SearchSymbolDTO(repo_id=status_dto.repo_id, query_text="UserRepository", query_type="EXACT"))

    # 5. Evaluate Performance & Accuracy
    # Exact Search (<5ms)
    t0 = time.perf_counter()
    exact_res = search_sym_uc.execute(SearchSymbolDTO(repo_id=status_dto.repo_id, query_text="UserRepository", query_type="EXACT"))
    ...
    assert exact_lat_ms < 5.0, f"Exact search latency {exact_lat_ms:.2f}ms exceeded target of 5.0ms"

    # Prefix Search (<10ms)
    t0 = time.perf_counter()
    prefix_res = search_sym_uc.execute(SearchSymbolDTO(repo_id=status_dto.repo_id, query_text="User", query_type="PREFIX"))
    prefix_lat_ms = (time.perf_counter() - t0) * 1000.0

    assert prefix_res.is_success
    assert prefix_lat_ms < 10.0, f"Prefix search latency {prefix_lat_ms:.2f}ms exceeded target of 10.0ms"

    # Autocomplete (<5ms)
    t0 = time.perf_counter()
    auto_res = auto_uc.execute(AutocompleteDTO(repo_id=status_dto.repo_id, prefix="User"))
    ...
    assert auto_lat_ms < 5.0, f"Autocomplete latency {auto_lat_ms:.2f}ms exceeded target of 5.0ms"
