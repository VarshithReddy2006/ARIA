...
def test_full_query_engine_end_to_end_pipeline(tmp_path: Path) -> None:
    ...
    subprocess.run(
        ["git", "commit", "-m", "Query test commit"], cwd=origin_dir, check=True
    )
    ...
    status_dto = reg_use_case.execute(
        RegisterRepositoryCommand(remote_url=str(origin_dir), name="query_origin")
    )
    ...
    # Warm-up lookup
    _ = find_def_uc.execute(
        FindDefinitionQueryDTO(repo_id=status_dto.repo_id, symbol_name="UserService")
    )

    t0 = time.perf_counter()
    def_res = find_def_uc.execute(
        FindDefinitionQueryDTO(repo_id=status_dto.repo_id, symbol_name="UserService")
    )
    ...
    assert def_lat_ms < 5.0, (
        f"Definition lookup latency {def_lat_ms:.2f}ms exceeded target of 5.0ms"
    )

    # Symbol Search
    srch_res = search_sym_uc.execute(
        SearchSymbolQueryDTO(repo_id=status_dto.repo_id, symbol_name="user")
    )
    ...
    assert dep_lat_ms < 30.0, (
        f"Dependency query latency {dep_lat_ms:.2f}ms exceeded target of 30.0ms"
    )
