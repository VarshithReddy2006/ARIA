...
def test_atomic_rebuild_publishes_complete_new_revision(chroma_test_dir) -> None:
    store = ChromaStore(persist_directory=chroma_test_dir)
    store.index_repository(
        "owner/repo", [_repository_chunk("old.py", "old")], [[0.1, 0.2, 0.3]]
    )
    store.index_repository(
        "owner/repo", [_repository_chunk("new.py", "new")], [[0.1, 0.2, 0.3]]
    )
...
def test_rename_vector_update_removes_old_path_and_indexes_new_path(
    chroma_test_dir,
) -> None:
    store = ChromaStore(persist_directory=chroma_test_dir)
    store.index_repository(
        "owner/repo",
        [_repository_chunk("old_name.py", "same content")],
        [[0.1, 0.2, 0.3]],
    )
...
def test_atomic_rebuild_keeps_old_revision_queryable_while_staging(
    chroma_test_dir, monkeypatch
) -> None:
    store = ChromaStore(persist_directory=chroma_test_dir)
    store.index_repository(
        "owner/repo", [_repository_chunk("old.py", "old")], [[0.1, 0.2, 0.3]]
