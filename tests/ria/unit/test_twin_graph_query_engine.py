"""Unit tests for TwinGraphQueryEngine (Phase 4)."""
...
def test_twin_graph_query_engine() -> None:
    ...
    engine = TwinGraphQueryEngine()
    node = engine.node_lookup(twin, "nonexistent")
    assert node is None

    edge = engine.edge_lookup(twin, "nonexistent")
    assert edge is None

    neighbours = engine.neighbour_lookup(twin, "nonexistent")
