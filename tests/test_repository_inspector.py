...
def _minimal_context(
    files=None, twin_overrides=None, kg_overrides=None
) -> InspectionContext:
...
class TestInspectionPacks:
    ...
    def test_security_inspector_flags_vulnerabilities(self):
        ...
        assert any(
            f.category == "security" and f.severity == "critical" for f in findings
        )
...
class TestConfidenceEngine:
    def test_boosts_confidence_with_more_evidence(self):
        ...
        rescored = engine.rescore(
            [finding], {"metadata": {"symbols_count": 10, "files_count": 5}}
        )
    ...
    def test_penalizes_no_entities_for_large_repo(self):
        ...
        rescored = engine.rescore(
            [finding], {"metadata": {"symbols_count": 100, "files_count": 50}}
        )
...
class TestInspectionRouter:
    ...
    def test_get_statistics_returns_404_when_no_report(self):
        response = client.get(
            "/api/repositories/ghost/nonexistent/inspection/statistics"
        )
        assert response.status_code == 404
    ...
    def test_get_latest_returns_report_when_cached(self):
        with tempfile.TemporaryDirectory():
            with patch(
                "backend.routers.inspection.repository_inspector"
            ) as mock_inspector:
