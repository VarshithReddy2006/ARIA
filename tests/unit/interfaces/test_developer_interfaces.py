...
def test_rest_api_server_endpoints() -> None:
    ...
    mock_sync.register_repository.return_value = MagicMock(
        is_success=True, repo_id="r1", status="SYNCED"
    )
    mock_sync.synchronize_repository.return_value = MagicMock(
        is_success=True, repo_id="r1", current_commit_sha="abc"
    )
    mock_search.search_symbol.return_value = MagicMock(
        is_success=True, results=MagicMock(payload=(1, 2, 3, 4, 5))
    )
    mock_query.find_definition.return_value = MagicMock(is_success=True, query_id="q1")
    mock_context.build_context.return_value = MagicMock(
        is_success=True, package_id="p1", total_tokens=150, content="{}"
    )
    mock_knowledge.answer_question.return_value = MagicMock(
        is_success=True, answer_text="Ans", is_grounded=True, grounding_score=0.98
    )

    server = RESTAPIServer(
        mock_sync, mock_search, mock_query, mock_context, mock_knowledge
    )
    ...
    resp = server.handle_request(
        "POST", "/repositories", {"remote_url": "https://repo.git", "name": "repo"}
    )
    assert resp.is_success
    ...
    # Search
    resp = server.handle_request(
        "POST", "/search", {"repo_id": "r1", "query_text": "main"}
    )
    assert resp.is_success
...
def test_mcp_server_tools() -> None:
    ...
    mock_search.search_symbol.return_value = MagicMock(
        is_success=True, results=MagicMock(payload=(1, 2))
    )
    mock_knowledge.answer_question.return_value = MagicMock(
        is_success=True, answer_text="Ans", is_grounded=True
    )
    ...
    res = mcp.invoke_tool(
        "ask_repository", {"repo_id": "r1", "question": "how to login?"}
    )
    assert res["is_success"]
...
def test_python_sdk_client() -> None:
    ...
    mock_search.search_symbol.return_value = MagicMock(
        is_success=True, results=MagicMock(payload=tuple(range(10)))
    )
    mock_knowledge.answer_question.return_value = MagicMock(
        is_success=True, answer_text="SDK Ans", is_grounded=True
    )
...
def test_cli_runner() -> None:
    ...
    mock_search.search_symbol.return_value = MagicMock(
        is_success=True, results=MagicMock(payload=(1,))
    )
...
def test_vscode_command_dispatcher() -> None:
    ...
    mock_knowledge.answer_question.return_value = MagicMock(
        is_success=True, answer_text="VS Code Ans"
    )

    disp = VSCodeCommandDispatcher(
        mock_search, mock_query, mock_context, mock_knowledge
    )
    res = disp.execute_command(
        "ria.askRepository", {"repo_id": "r1", "question": "auth?"}
    )
    assert res["is_success"]
